...
static void Cipher(state_t* state, const uint8_t* RoundKey, unsigned Nr)
{
  uint8_t round = 0;
...
void AES_ECB_encrypt(const struct AES_ctx* ctx, uint8_t* buf)
{
    ...
    Cipher((state_t*)buf, ctx->RoundKey, ctx->CipherRounds);
}

void AES_ECB_decrypt(const struct AES_ctx* ctx, uint8_t* buf)
{
    // The next function call decrypts the PlainText with the Key using AES algorithm.
    InvCipher((state_t*)buf, ctx->RoundKey, ctx->CipherRounds);
}
...
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, uint32_t length)
{
  ...
  for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi)
  {
    if (bi == AES_BLOCKLEN) /* we need to regen xor compliment in buffer */
    {
      ...
      Cipher((state_t*)buffer,ctx->RoundKey, ctx->CipherRounds);
